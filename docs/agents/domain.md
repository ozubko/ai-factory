...
├── CONTEXT.md
├── docs/adr/
│   ├── 0001-pluggable-agent-backend.md
│   └── 0002-git-only-isolation-v1.md
└── src/
```
...
synonyms the glossary explicitly avoids (each term lists them under `_Avoid_`).
...
> _Contradicts ADR-0003 (propose, never land) — but worth reopening because…_
